...
class AdvancedChatAppRunner(WorkflowBasedAppRunner):
    ...
    def run(self) -> None:
        ...
        if self.application_generate_entity.single_iteration_run:
            ...
            gree_token = ''
            if self._gree_mail:
                redis_key = get_redis_key(self._gree_mail)
                redis_value = redis_client.get(redis_key)
                if redis_value:
                    import json
                    user_info_dict = json.loads(redis_value)
                    user_info = UserInfo(** user_info_dict)
                    gree_token = user_info.Token
            ...
            system_inputs = {
                ...
                SystemVariableKey.GREE_MAIL: self._gree_mail,
                SystemVariableKey.GREE_TOKEN: gree_token,
            }
