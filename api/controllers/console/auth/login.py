...
# 根据token登录
class GreeTokenLogoutApi(Resource):
    @setup_required
    def get(self):
        parser = reqparse.RequestParser()
        parser.add_argument("authcode", type=str, required=True, location="args", help="格力authcode单点登录")
        args = parser.parse_args()
        token_pair = GreeSsoService.gree_login_by_token(args['authcode'])
        return {"result": "success", "data": token_pair.model_dump()}


# 根据authcode获取mail
class GreeAuthCodeGetMailApi(Resource):
    def get(self):
        parser = reqparse.RequestParser()
        parser.add_argument("authcode", type=str, required=True, location="args", help="格力authcode单点登录")
        args = parser.parse_args()
        gree_mail = GreeSsoService.gree_authcode_get_mail(args['authcode'])
        return {"gree_mail": gree_mail}


#  根据authcode和appcode创建临时登录
class GreeTokenPassportApi(Resource):

    def get(self):
        parser = reqparse.RequestParser()
        parser.add_argument("authcode", type=str, required=True, location="args", help="格力authcode单点登录")
        parser.add_argument("appcode", type=str, required=True, location="args", help="appcode应用id")
        args = parser.parse_args()
        token_passport = GreeSsoService.gree_passport_by_token(args['authcode'], args['appcode'])
