...
export const loginByToken: Fetcher<LoginResponse, { url: string }> = ({ url }) => {
  return get(url) as Promise<LoginResponse>
}

export const passportByToken: Fetcher<{ access_token: string }, { url: string }> = ({ url }) => {
  return get(url) as Promise<{ access_token: string }>
}

export const mailByToken: Fetcher<{ gree_mail: string }, { url: string }> = ({ url }) => {
  return get(url) as Promise<{ gree_mail: string }>
}
